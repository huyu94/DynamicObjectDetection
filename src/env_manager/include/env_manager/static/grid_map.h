...
struct MappingData
{
  Eigen::Vector3i center_last3i_; // 中心的idx，以上一次的摄像机位置为中心
  ...
  Eigen::Vector3d ringbuffer_lowbound3d_; // 下界double
  Eigen::Vector3i ringbuffer_lowbound3i_; // 下界int
  Eigen::Vector3d ringbuffer_upbound3d_; // 上界double
  Eigen::Vector3i ringbuffer_upbound3i_; // 下界int
  // Eigen::Vector3d ringbuffer_size3d_;
  Eigen::Vector3i ringbuffer_size3i_; // ringbuffer的voxel size 
  // 膨胀部分
  Eigen::Vector3i ringbuffer_inf_origin3i_; //
  Eigen::Vector3d ringbuffer_inf_lowbound3d_; // 膨胀后的ringbuffer下界 double
  Eigen::Vector3i ringbuffer_inf_lowbound3i_; // 膨胀后的ringbuffer下界 int
  Eigen::Vector3d ringbuffer_inf_upbound3d_; // 膨胀后的ringbuffer上界 double
  Eigen::Vector3i ringbuffer_inf_upbound3i_; // 膨胀后的ringbuffer上界 int
  Eigen::Vector3i ringbuffer_inf_size3i_; // 膨胀后的ringbuffer的voxel size
