...
void DspMap::initMap(ros::NodeHandle& nh)
{
    ...
    /********************set hype-parameters******************************/
    node_ = nh;
    node_.param("dsp_map/occupied_thresh",occupied_thresh_,-1.0);
    ...
    int pose_type = 1;
    node_.param("dsp_map/occupied_thresh",occupied_thresh_,0.6);
    ROS_INFO("dsp_map/occupied_thresh: %f",occupied_thresh_);
   ...
   /*** Set map parameters **/
    ...
    /*init callback*/
    pointcloud_sub_.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(node_,"dsp_map/cloud",1));
    if(pose_type == POSE_STAMPED)
    {
        // ROS_INFO("POSE_TYPE:%d",pose_type);
        pose_sub_.reset(new message_filters::Subscriber<geometry_msgs::PoseStamped>(node_,"dsp_map/pose",1));
    ...
    {
        odom_sub_.reset(new message_filters::Subscriber<nav_msgs::Odometry>(node_,"dsp_map/odom",1));
    ...
    map_pub_ = node_.advertise<sensor_msgs::PointCloud2>("/dsp_map/occupancy_map",1);
                                    ...
                                    const geometry_msgs::PoseStampedConstPtr &pose)
{
    ...
    // **/
    // getOccupancyMapWithFutureStatus(occupied_num, cloud_to_publish, &future_status[0][0]);
...
/// @brief 计算每个栅格的未来状态，存储在future status里面，将当前占据信息和未来占据信息发布出去
/// @param obstacles_num 
/// @param cloud 
/// @param future_status 
/// @param threshold 
void DspMap::getOccupancyMapWithFutureStatus(){
    int address;
    Index localIndex;
    Position position;
    pcl::PointCloud<pcl::PointXYZRGB> map_cloud;
    pcl::PointCloud<pcl::PointXYZRGB> future_map_cloud;
    sensor_msgs::PointCloud2 future_cloud_to_pub;
    // int z_index_to_show = MAP_HEIGHT_VOXEL_NUM / 2 - 1; // layer
    int z_index_to_show = 0;
    for(int i=0;i<getMapSize()(0);i++)
    {
        for(int j=0;j<getMapSize()(1);j++)
        {
            for(int k=0;k<getMapSize()(2);k++)
            {
                ...
                if(voxels_objects_number[address][0] > occupied_thresh_){
                    pcl::PointXYZRGB pcl_point;
                    pcl_point.x = position.x();
                    pcl_point.y = position.y();
                    pcl_point.z = position.z();
                    z_height = pcl_point.z;
                    map_cloud.push_back(pcl_point);
                }

                for(int n=0;n<PREDICTION_TIMES; ++n)
                {
                    future_status[address][n] = voxels_objects_number[address][n+4];
                }
                // cout << z_height << endl;
                //预测
                if(k == z_index_to_show)
                {
                    // cout <<" here " << endl;
                    float weight_this = 0;
                    for(int n=0; n < PREDICTION_TIMES; ++n)
                    {
                        float z_offset = (float) (n+1) * 12.f;
                        weight_this = future_status[address][n];
                        int r,g,b;
                        pcl::PointXYZRGB p_future;
                        colorAssign(r,g,b,weight_this,0.f,0.1f,1);

                        p_future.x = position(0);
                        p_future.y = position(1)+ z_offset;
                        p_future.z = 0;
                        p_future.r = r;
                        p_future.g = g;
                        p_future.b = b;
                        future_map_cloud.push_back(p_future);
                    }
                }
                // cout << "z_height : " << z_height << endl;
    ...
    // cout << "future map size : "  << future_map_cloud.size() << endl;

    sensor_msgs::PointCloud2 map_cloud_to_pub;
    sensor_msgs::PointCloud2 future_map_cloud_to_pub;
    pcl::toROSMsg(map_cloud, map_cloud_to_pub);
    pcl::toROSMsg(future_map_cloud, future_map_cloud_to_pub);
    map_cloud_to_pub.header.frame_id = "world";
    map_cloud_to_pub.header.stamp = ros::Time::now();
    future_map_cloud_to_pub.header.frame_id = "world";
    future_map_cloud_to_pub.header.stamp = ros::Time::now();
    map_pub_.publish(map_cloud_to_pub);
    future_map_pub_.publish(future_map_cloud_to_pub);
            ...
            const Rotation& sensorQuaternion)  /// also requires point cloud and velocity
{
    ...
    // ROS_INFO("MAP update successful!!");
...
int DspMap::getVoxelState(const Eigen::Vector3d& pos) const
{
    ...
    if(voxels_objects_number[address][0] > occupied_thresh_) return 1;
    else return 0;
    // int res = voxels_objects_number[address][0] > 0.0 ? 1 : 0;
    // ROS_INFO("%f\t%f\t%f\t%f",posf[0],posf[1],posf[2],res);
}

int DspMap::getVoxelStateWithFutureRisk(const Eigen::Vector3d& pos,double &future_risk)
{
    Position posf = pos.cast<float>();
    Index index;
    getIndex(index,posf);
    ...
    for(int n=0;n<PREDICTION_TIMES;n++)
    {
        future_risk += future_status[address][n];
    }
    if(voxels_objects_number[address][0] > occupied_thresh_) return 1;
    else return 0;
}

double DspMap::getVoxelFutureRisk(const Eigen::Vector3d& pos) const
{
    Position posf = pos.cast<float>();
    Index index;
    getIndex(index,posf);
...
void DspMap::colorAssign(int &r, int &g, int &b, float v, float value_min=0.f, float value_max=1.f, int reverse_color=0)
{
