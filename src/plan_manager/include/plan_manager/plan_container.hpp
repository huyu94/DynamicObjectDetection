...
namespace fast_planner
{
    ...
    public:
        ...
        {
            if(t >= -1e-3 && t <= local_start_time_) // 未到局部轨迹
            {
                return global_traj_.evaluate(t - time_increase_ + last_time_inc_);
            }
            else if(t >= local_end_time_ && t <= global_duration_ + 1e-3) // 超出全局轨迹时间
            {
            ...
            else
            {
                ...
                return local_traj_[0].evaluateDeBoorT(tm + t - local_start_time_);
            }
        }
